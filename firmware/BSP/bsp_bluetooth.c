...
void BLE_Send_Bit(unsigned char ch)
{
		...
		while( DL_UART_isBusy(UART_HC_05_INST) == true );
		//发送单个字符
		DL_UART_Main_transmitData(UART_HC_05_INST, ch);
}  
...
{
				DL_GPIO_enableOutput(GPIOB, DL_GPIO_PIN_2);
        //清除串口中断标志
        NVIC_ClearPendingIRQ(UART_HC_05_INST_INT_IRQN);
        //使能串口中断
        NVIC_EnableIRQ(UART_HC_05_INST_INT_IRQN);
				DL_UART_enableInterrupt(UART_HC_05_INST, DL_UART_INTERRUPT_RX);
...
{
		//如果产生了串口中断
		switch( DL_UART_getPendingInterrupt(UART_HC_05_INST) )
		{
			case DL_UART_IIDX_RX://如果是接收中断
						...
						{
								BLERX_BUFF[BLERX_LEN++] = DL_UART_Main_receiveData(UART_HC_05_INST); // 接收数据
						}
						else
						{
								uint8_t temp = DL_UART_Main_receiveData(UART_HC_05_INST); // 不保存数据
						}
